...
class _OrdersLocationState extends ConsumerState<OrdersLocation> {
  ...
  void _updateMarkers(List<OrderModel> orders) {
    ...
    for (var order in orders) {
      // In a real app, you would use the actual coordinates from the order
      // For demonstration, generating coordinates around the restaurant
      final double lat = (_restaurantLocation?.latitude ?? 37.422) + 
                         (orders.indexOf(order) * 0.01 - 0.02);
      final double lng = (_restaurantLocation?.longitude ?? -122.084) + 
                         (orders.indexOf(order) * 0.01 - 0.02);
      ...
      final marker = Marker(
        ...
        infoWindow: InfoWindow(
          ...
          snippet: '${order.customerName} - ${_getStatusText(order.status)}',
        ),
                    ...
                    onPressed: () {
                      if (order.status == OrderStatus.preparing) {
                        ref.read(orderManagementProvider.notifier)
                            .updateOrderStatus(order.id, OrderStatus.readyForDelivery);
                      } else {
                        ref.read(orderManagementProvider.notifier)
